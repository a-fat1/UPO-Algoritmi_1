...
void *binary_search(const void *key, const void *base, size_t num_elem, size_t elem_size, int (*compar)(const void *, const void *)) {
    ...
    /* Soluzione */
    size_t left = 0, right = num_elem - 1, mid;
    const void *mid_elem;

    while (left <= right) {
        mid = left + (right - left) / 2;
        mid_elem = (const char *)base + (mid * elem_size);

        if (compar(key, mid_elem) < 0)
            right = mid - 1;    // La chiave si trova nel sottoarray sinistro
        else if (compar(key, mid_elem) > 0)
                left = mid + 1;     // La chiave si trova nel sottoarray destro
            else
                return (void *)mid_elem;    // Elemento trovato
    }
